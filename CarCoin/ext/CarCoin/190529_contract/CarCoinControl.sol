...
contract CarCoinControl is CarCoinDatabase {
    ...
    function setPermission(address target, uint8 permit) external AllPermitted NotZeroAddr(target){
        require(target != msg.sender
               && permit <= 4);
               
        permission[target] = permit;
    }
    function permitAll(address target) external OnlyCEO NotZeroAddr(target){
        require(target != msg.sender);
